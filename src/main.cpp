...
#ifdef OS_WIN
static const char * sDirSeparator = "\\";
#else
static const char * sDirSeparator = "/";
#endif
...
public:
	...
	virtual size_t ReadFile(void* file, void* lpBuf, size_t uiBufSize) override
	{
		...
		long length = file_response.tellg();
		...
		size_t file_length = static_cast<size_t>(length);
		size_t chunk_size = std::min(uiBufSize, file_length - _file_pos);

		file_response.seekg(static_cast<long>(_file_pos), std::ios::beg);
		file_response.read(static_cast<char*>(lpBuf), static_cast<long>(chunk_size));
		file_response.close();

		_file_pos += chunk_size;

		return file_length - _file_pos;
	}
	...
	{
#ifdef OS_WIN
		return !CreateDirectoryW((wchar_t *)dir, NULL) || GetLastError() == ERROR_ALREADY_EXISTS;
#else
...
{
	file_mpd = std::ifstream(info_path+sDirSeparator+"manifest.mpd");
...
int main(int argc, char *argv[])
{
	...
	std::string nom_search = std::string("/") + std::string(sDirSeparator);
	std::string nom_fragment = encrypted_file.substr(encrypted_file.find_last_of(nom_search)+1);
	...
	{
		printf("[ERROR] Unknown stream id.\n");
		return -1;
	}

	std::string nom_video = nom_fragment.substr(0,nom_fragment.find("_"));
	printf("[INFO] File: %s\n", nom_fragment.c_str());
	printf("[INFO] Info path: %s\n", info_path.c_str());
	printf("[INFO] Decrypted path: %s\n", decrypted_path.c_str());

	file_fragment = std::ifstream(encrypted_file, std::ios::binary);
	file_decrypted_data = std::ofstream(decrypted_path+sDirSeparator+nom_video+"_track_"+stream_id_str, std::ios::binary | std::ios::trunc);

	MyHost host;
	profile_path = info_path+sDirSeparator+std::to_string(std::time(nullptr))+sDirSeparator;
	printf("[DEBUG] Profile path: %s\n", profile_path.c_str());
	host.CreateDirectory(profile_path.c_str());

	// --------------------------------------------------------------------------------------------------------------
	// --------------------------------------------------------------------------------------------------------------

	printf("[DEBUG] Reading license_key.txt...\n");
	
	std::ifstream file_lic(info_path+sDirSeparator+"license_key.txt");
	std::stringstream buffer;
...
#ifdef NDEBUG
	std::string lib_wvdecrypter_path = std::string(dirname(strdup(argv[0]))) + "/wvdecrypter/libssd_wv.dylib";
#else
	std::string lib_wvdecrypter_path = std::string(dirname(strdup(argv[0]))) + "/wvdecrypter/libssd_wvd.dylib";
#endif // NDEBUG

	void * mod(dlopen(lib_wvdecrypter_path.c_str(), RTLD_LAZY));
	if (mod)
	{
        printf("[DEBUG] Creating decryptor instance...\n");
	...
	{
		printf("[ERROR] Can't open manifest\n");
		return -1;
	}

	if (adaptiveTree_->empty())
	{
		printf("[ERROR] Can't parse manifest\n");
		return -1;
	}

    printf("[INFO] Parsing manifest...\n");
	printf("[INFO] Found %d streams\n", adaptiveTree_->periods_[0]->adaptationSets_.size());

	if (adaptiveTree_->current_period_->encryptionState_ == adaptive::AdaptiveTree::ENCRYTIONSTATE_ENCRYPTED)
	{
		printf("[ERROR] Unable to handle decryption. Unsupported!\n");
		return -1;
	}
	...
	{
		printf("[INFO] Entering encryption section\n");

		if (license_key_.empty())
		{
			printf("[ERROR] Invalid license_key\n");
			return -1;
		}
		if (!decrypter_)
		{
			printf("[ERROR] No decrypter found for encrypted stream\n");
			return -1;
		}
		// Что за конфиг - не понятно, но он требуется как аргумент к OpenDRMSystem, поэтому пока просто поставлю 1
		const uint8_t config = 1;
		if(!decrypter_->OpenDRMSystem(license_key_.c_str(), AP4_DataBuffer(), config))
		{
			printf("[ERROR] OpenDRMSystem failed");
			return -1;
		}
		...
		for (size_t ses(1); ses < cdm_sessions_.size(); ++ses)
		{
			if (adaptiveTree_->current_period_->psshSets_[ses].pssh_ == "FILE")
			{
				if (license_data_.empty())
				{
					...
					stream.enabled = true;
					stream.stream_.start_stream(0, width_, height_, false);
					...
					if (movie == NULL)
					{
						printf("[ERROR] No MOOV in stream!\n");
						stream.disable();
						return -1;
					}
							...
							if (adaptiveTree_->current_period_->psshSets_[ses].defaultKID_.empty())
							{
								...
								else if (AP4_Track *track = movie->GetTrack(TIDC[stream.stream_.get_type()]))
								{
									...
									if (m_protectedDesc->GetSchemeInfo() && (schi = m_protectedDesc->GetSchemeInfo()->GetSchiAtom()))
									{
										...
										if (tenc)
											adaptiveTree_->current_period_->psshSets_[ses].defaultKID_ = std::string((const char*)tenc->GetDefaultKid());
										else
										{
											...
											if (piff)
												adaptiveTree_->current_period_->psshSets_[ses].defaultKID_ = std::string((const char*)piff->GetDefaultKid());
										}
									}
								}
							}
					...
					{
						printf("[ERROR] Could not extract license from video stream (PSSH not found)\n");
						stream.disable();
						return -1;
					}
					stream.disable();
			...
			else
			{
				...
				b64_decode(adaptiveTree_->current_period_->psshSets_[ses].pssh_.data(), adaptiveTree_->current_period_->psshSets_[ses].pssh_.size(), init_data.UseData(), init_data_size);
				init_data.SetDataSize(init_data_size);
			}

			CDMSESSION &session(cdm_sessions_[ses]);
			const char *defkid = adaptiveTree_->current_period_->psshSets_[ses].defaultKID_.empty() ? nullptr : adaptiveTree_->current_period_->psshSets_[ses].defaultKID_.data();
			...
			{
				printf("[ERROR] Initialize failed (SingleSampleDecrypter)\n");
	...
	// --------------------------------------------------------------------------------------------------------------
	// --------------------------------------------------------------------------------------------------------------
	printf("[INFO] OpenStream(%d)\n", stream_id);

	STREAM *stream(GetStream(stream_id));

	if (!stream || stream->enabled)
	{
		printf("[INFO] Stream id not found.\n");
		return -1;
	}

	stream->enabled = true;

	stream->stream_.start_stream(~0, GetVideoWidth(), GetVideoHeight(), false);
	...
	printf("[INFO] Selecting stream with conditions: w: %u, h: %u, bw: %u\n",
	...
	{
		printf("[ERROR] Unable to select stream!\n");
		stream->disable();
		return -1;
	}
	...
	if (!track)
	{
		printf("[ERROR] No suitable track found in stream\n");
		stream->disable();
		return -1;
	}
