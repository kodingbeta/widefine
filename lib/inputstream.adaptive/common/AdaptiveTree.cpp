...
namespace adaptive
{
  ...
  {
    printf("AdaptiveTree::PreparePaths - URL: %s\n", url.c_str());
    printf("AdaptiveTree::PreparePaths - manifestUpdateParam: %s\n", manifestUpdateParam.c_str());
    ...
    if (paramPos == std::string::npos)
    {
      Log(LOGLEVEL_ERROR, "[ERROR] Invalid mpdURL: / expected. Given URL: %s", url.c_str());
      return false;
    }
